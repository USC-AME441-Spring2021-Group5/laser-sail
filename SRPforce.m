function F_SRP = SRPforce(R,N,theta_prime,q)
...
phi = 1366;      %solar constant adjusted for actual distance from the sun [W/m^2]
...
for j = alpha(1,:)
    ...
    SRPvec(1,i) = (phi/c)*A_dy*(1+q).*cosd(j).*cosd(j); 
    SRPvec(2,i) = (phi/c)*A_dy*(1+q).*cosd(j).*sind(j); 
    i = i + 1;
end
...
F_SRP = [norm(SRPvecTOT)*cosd(theta_prime) norm(SRPvecTOT)*sind(theta_prime)];
